# WozMania: an Apple ][ emulator for ARM 64


<a name="toc"/>

## Table of Contents

 * [How to Use](usage.md)
    * [Running WozMania](usage.md#run)
    * [Using the Keyboard](usage.md#keyboard)
    * [Using the Floppy Disks](usage.md#floppy)
    * [Using the Language Card](usage.md#language)
    * [Using the 80 Column Card](usage.md#v80)
 * [How to Debug](debug.md)
    * [Debugging with gdb](debug.md#gdb)
    * [Helper Routines](debug.md#helpers)
 * [Performance Considerations](performance.md)
    * [Coding Style](performance.md#style)
    * [Processor Accelerations](performance.md#processor)
    * [Input-output Accelerations](performance.md#input-output)
    * [Not Implemented Accelerations](#not-implemented)
 * [Bibliography](bibliography.md#biblio)
